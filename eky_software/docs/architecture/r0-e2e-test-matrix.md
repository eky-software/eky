...
| UPDATE-LOCAL-001 | P0; packaged-smoke, electron-e2e, critical | Validoitu paikallinen pilot-paketti; pre-update-piste, shutdown, install ja first start | Uusi build hyväksytään vasta migraatioiden ja healthin jälkeen | Sama app identity/userData; yksi journaliketju | Installer command, polku, session tai business data | planned-r0 |
| UPDATE-FAILURE-001 | P0; packaged-smoke, electron-e2e, fault, recovery | Hash-, spawn-, migration-, integrity-, readiness- tai health-failure | Päivitys ei hyväksyydy; business- ja tarvittaessa binary-rollback tehdään | Ei reverse SQL -migraatiota; uusi runtime ei avaudu business-tilaan | Paketti, polku, process output tai raw error | planned-r0 |
