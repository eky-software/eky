...
| BACKUP-PORTABLE-001 | P0; integration, electron-e2e, packaged-smoke, security | Edustava synteettinen profiili; luo salattu `.ekybackup`, tarkista ja palauta tyhjään asennukseen | Business- ja document-data täsmäävät restartin jälkeen | Ei plaintext-vaihtoehtoa; container, manifesti, checksumit ja uusi session validoidaan | Salasana, avain, payload, manifesti tai polku rendererille/lokiin | planned-r0 |
| BACKUP-TAMPER-001 | P0; integration, electron-e2e, security | Väärä salasana sekä muutettu header, ciphertext, tag tai manifesti | Palautus estyy yhtenäisellä turvallisella virheellä | Aktiivinen profiili ja pre-restore-piste eivät muutu | Salausvaiheen yksityiskohta, salasana tai avainmateriaali | planned-r0 |
| RESTORE-ROLLBACK-001 | P0; packaged-smoke, electron-e2e, fault, recovery | Keskeytä staging, aktivointi, migraatio, readiness tai health | Aktiivinen profiili säilyy tai pre-restore-piste palautuu terveeksi | Journal on idempotentti; vanha session torjutaan | Profiilipolku, business data tai raw failure | planned-r0 |
| RECOVERY-ROTATION-001 | P0; integration, electron-e2e, recovery | Healthy/unclean startup, 24 h -raja, levybudjetti ja aktiivinen pre-update-piste | Vain terveestä profiilista tehdään piste; newest good ja aktiivinen piste säilyvät | `safeStorage`-failure ei tuota plaintextia | Data-avain, salattu payload tai paikallinen polku | planned-r0 |
