...
| Local Company Workspace Registry / Coordinator | W1-W4 runtime foundation implemented; W5 UI and W6 packaged release gate pending | Täsmällinen installation-scoped registry v1, opaque `workspaceId`, johdettu workspace-root, yksi aktiivinen osoitin, erilliset operation-journalit sekä hallittu create/import/replace/switch/adoption-lifecycle | Yrityksen business-data, `companyId`, käyttäjän antama storage locator, backup-container, salaisuuden arvo, pilvitenant tai update-binaarit | Electron mainin production-startup ratkaisee/adoptoi aktiivisen workspacen ennen sessionia ja backendia; W5:n renderer-capability puuttuu vielä | Ei vielä renderer-capabilitya; W5 lisää turvallisen workspace-listauksen sekä rajatut create/import/active-replace/switch-capabilityt ilman polkuja tai lineagea rendererille | Profile Protection, Runtime Trust, backend lifecycle, workspace-bound adapterit, `WorkspaceMaintenanceLease` ja installation-scoped update gate kapeiden porttien kautta | Registry lukee strict v1 -minimimetadatan; bootstrap/adoption palauttaa vain validoidun readiness- ja lineage-tuloksen, ei SQLite-kahvaa | Maintenance lease sulkee vanhat kirjoitukset, ikkunat, backendin, SQLite-kahvat, sessionin ja adapterit ennen candidatea; enintään yksi business-SQLite-owner | Ei vielä renderer-capabilitya; tuleva trusted main frame saa käyttää vain mainin antamaa workspaceId:tä ja business-permissionit tarkistetaan uudessa ActorContextissa | Ei business auditia | Workspace lifecycle -eventit planned ilman polkua, companyId:tä, lineagea tai business-dataa | Ei | Planned turvallinen installation/runtime-projektio | Ei registryä, lineagea, polkua tai business-dataa | Planned failure/security-minimointi | Registry, active pointer, operation-journalit ja update state excluded; yksi workspace = yksi immutable companyId/SQLite/artifact-backup; secret ja archive config device-local | Kukin työtila käyttää omien moduuliensa artifact-catalogeja | Tyhjä candidate validoidaan ennen julkaisua; import uutena vain rekisteröimättömälle lineagelle; replace vain aktiiviselle exact-lineage-työtilalle; legacy-adoptio copy -> validate -> atomic publish | Electron mainin startup/runtime-composition implemented; web saa vasta W5:ssä capabilityn; cloud/mobile eivät käytä local registryä | W1-W4 registry-, coordinator-, adoption-, startup-, boundary- ja A -> B -> A -testit toteutettu; full `WORKSPACE-ADOPTION/ISOLATION/SECRETS/LIFECYCLE/UPDATE/ARCHIVE-001` packaged proof W6:ssa; delete deferred W7 |
...
rendererille. Hallintacapability ja UI kuuluvat W5:een; active/passive N ->
