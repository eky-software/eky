...
| Local Company Workspace Registry / Coordinator | W1-W4 runtime foundation implemented; W5A main-only management foundation in progress; W5B UI and W6 packaged release gate pending | Täsmällinen installation-scoped registry v1, opaque `workspaceId`, johdettu workspace-root, yksi aktiivinen osoitin, erilliset operation-journalit sekä hallittu create/import/replace/switch/adoption/rename-lifecycle | Yrityksen business-data, `companyId`, käyttäjän antama storage locator, backup-container, salaisuuden arvo, pilvitenant tai update-binaarit | Electron mainin production-startup ratkaisee/adoptoi aktiivisen workspacen ennen sessionia ja backendia; W5A lisää vain mainin sisäisen management-palvelun ja production-adapterit | Ei vielä renderer-capabilitya; W5B lisää turvallisen workspace-listauksen sekä rajatut create/import/active-replace/switch/rename-capabilityt ilman polkuja tai lineagea rendererille | Profile Protection, Runtime Trust, backend lifecycle, workspace-bound adapterit, yksi main-owned `WorkspaceMaintenanceLease` ja installation-scoped update gate kapeiden porttien kautta | Registry lukee strict v1 -minimimetadatan; bootstrap/adoption palauttaa vain validoidun readiness- ja lineage-tuloksen, ei SQLite-kahvaa | Maintenance lease sulkee vanhat kirjoitukset, ikkunat, backendin, SQLite-kahvat, sessionin ja adapterit ennen candidatea; enintään yksi business-SQLite-owner | Ei vielä renderer-capabilitya; tuleva trusted main frame saa käyttää vain mainin antamaa workspaceId:tä ja business-permissionit tarkistetaan uudessa ActorContextissa | Ei business auditia | Workspace lifecycle -eventit ilman polkua, companyId:tä, lineagea tai business-dataa | Ei | W5A:n rajattu installation/runtime-projektio | Ei registryä, lineagea, polkua tai business-dataa | W5A:n turvallinen failure/security-minimointi | Registry, active pointer, operation-journalit ja update state excluded; yksi workspace = yksi immutable companyId/SQLite/artifact-backup; secret ja archive config device-local | Kukin työtila käyttää omien moduuliensa artifact-catalogeja | Tyhjä candidate validoidaan ennen julkaisua; import uutena vain rekisteröimättömälle lineagelle; replace vain aktiiviselle exact-lineage-työtilalle; legacy-adoptio copy -> validate -> atomic publish | Electron mainin startup/runtime-composition implemented; W5A pysyy mainin sisäisenä ja web saa capabilityn vasta W5B:ssä; cloud/mobile eivät käytä local registryä | W1-W4 registry-, coordinator-, adoption-, startup-, boundary- ja A -> B -> A -testit toteutettu; W5A lisää management/maintenance/production-adapteritestit; full `WORKSPACE-ADOPTION/ISOLATION/SECRETS/LIFECYCLE/UPDATE/ARCHIVE-001` packaged proof W6:ssa; delete deferred W7 |
...
rendererille. Mainin sisäinen management-foundation kuuluu W5A:han ja
hallintacapability sekä UI W5B:hen; active/passive N ->
