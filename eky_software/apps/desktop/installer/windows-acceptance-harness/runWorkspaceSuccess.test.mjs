...
import { runWorkspaceSuccess, workspaceSuccessCommandFailureDetails } from './runWorkspaceSuccess.mjs';
...
for (const mode of ['completed', 'preconditionFailed', 'prepareFailed', 'launchFailed', 'supervisorMissing',
  ...
  'profileChanged', 'artifactChanged', 'fixtureRemovalFailed']) {
  test(`workspace command preserves outcomes and exact cleanup: ${mode}`, async (t) => {
    let root, context;
    ...
    const options = {
      ...
      startSupervisor: (path) => {
        ...
        return { child: { exitCode: 0, signalCode: null }, completion: readFile(path, 'utf8').then((source) => {
          ...
          assert.match(value.arguments[0], /runWorkspaceSuccessWorker.mjs$/);
          return 0;
        }) };
      },
      ...
      readScenario: async () => {
        ...
        return { schemaVersion: 1, scenario: context.request.scenario, runNonce: context.request.runNonce,
          ...
          resultCode: 'workspaceSuccessCompleted', errorCode: null, failedPhase: null, completedPhases: [...WORKSPACE_SUCCESS_PHASES] };
      },
      verifySemantic: async () => {
        if (mode === 'businessFailed') throw new Error('private company');
        return { status: 'completed', resultCode: 'workspaceSemanticProofValidated' };
      },
    ...
    const run = () => runWorkspaceSuccess(['--artifact-descriptor', resolve('workspace-success-artifact.json'),
      '--expected-descriptor-sha256', 'b'.repeat(64), '--expected-build-revision', 'a'.repeat(40)], options);
    let outcome;
    if (mode === 'completed') {
      await assert.doesNotReject(async () => { outcome = await run(); });
      assert.equal(outcome.status, 'completed');
      assert.equal(outcome.semanticProofResultCode, 'workspaceSemanticProofValidated');
    } else {
