import { basename, isAbsolute, join, resolve } from 'node:path';

import {
  WORKSPACE_BACKUP_IMPORT_JOURNAL_UNAVAILABLE,
  WorkspaceBackupImportJournalStoreError,
} from './workspaceBackupImportJournalError.js';

export const WORKSPACE_BACKUP_IMPORT_JOURNAL_FILE_NAME =
  'workspace-backup-import-journal-v1.json';
export const WORKSPACE_BACKUP_IMPORT_JOURNAL_NEXT_FILE_NAME =
  'workspace-backup-import-journal-v1.json.next';
export const WORKSPACE_BACKUP_IMPORT_JOURNAL_BACKUP_FILE_NAME =
  'workspace-backup-import-journal-v1.json.backup';

export interface WorkspaceBackupImportJournalPaths {
  readonly directoryPath: string;
  readonly currentPath: string;
  readonly nextPath: string;
  readonly backupPath: string;
}

export function createWorkspaceBackupImportJournalPaths(
  installationRoot: string,
  filePath: string,
): Readonly<WorkspaceBackupImportJournalPaths> {
  if (
    typeof installationRoot !== 'string' ||
    typeof filePath !== 'string' ||
    installationRoot.includes('\0') ||
    filePath.includes('\0') ||
    !isAbsolute(installationRoot) ||
    !isAbsolute(filePath) ||
    basename(filePath) !== WORKSPACE_BACKUP_IMPORT_JOURNAL_FILE_NAME
  ) {
    throw new WorkspaceBackupImportJournalStoreError(
      WORKSPACE_BACKUP_IMPORT_JOURNAL_UNAVAILABLE,
    );
  }
  const directoryPath = resolve(installationRoot);
  const currentPath = join(
    directoryPath,
    WORKSPACE_BACKUP_IMPORT_JOURNAL_FILE_NAME,
  );
  if (!pathsAreEqual(filePath, currentPath)) {
    throw new WorkspaceBackupImportJournalStoreError(
      WORKSPACE_BACKUP_IMPORT_JOURNAL_UNAVAILABLE,
    );
  }
  return Object.freeze({
    directoryPath,
    currentPath,
    nextPath: join(
      directoryPath,
      WORKSPACE_BACKUP_IMPORT_JOURNAL_NEXT_FILE_NAME,
    ),
    backupPath: join(
      directoryPath,
      WORKSPACE_BACKUP_IMPORT_JOURNAL_BACKUP_FILE_NAME,
    ),
  });
}

function pathsAreEqual(firstPath: string, secondPath: string): boolean {
  const first = resolve(firstPath);
  const second = resolve(secondPath);
  return process.platform === 'win32'
    ? first.toLowerCase() === second.toLowerCase()
    : first === second;
}
