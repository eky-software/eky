...
export interface BackupContainerSourceEntry
...
> {
  ...
  const plaintextLength = calculatePayloadLength([
    manifestDescriptor,
    ...descriptors,
  ]);
...
async function inspectSourceEntry(
  entry: BackupContainerSourceEntry,
): Promise<BigIntStats> {
...
function compareEntries(
  first: BackupContainerSourceEntry,
  second: BackupContainerSourceEntry,
): number {
  return (
