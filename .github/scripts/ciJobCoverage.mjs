...
export function requiredCiJobs(plan) {
  ...
  return {
    ...
    cleanLifecycle: [job('Build Windows acceptance artifact once'), ...repeat('Verify clean lifecycle run',
    ...
    legacyUpgrade: [...['core', 'commands'].flatMap((group) => repeat(`V2.5 ${group} contracts run`,
      ['Build existing supervisor once', `Run legacy ${group} contracts`])), job('Build one historical source and target phase artifact'),
