...
export function requiredCiJobs(plan) {
  ...
  return {
    ...
    cleanLifecycle: [job('Build Windows acceptance artifact once',
      ...
      ['Build acceptance supervisor', 'Run supervised clean lifecycle once', 'Reverify downloaded artifact bytes'])],
    upgradeRollback: [job('Build upgrade acceptance artifact once'), ...repeat('Verify upgrade and rollback run',
      ['Build acceptance supervisor', 'Run supervised upgrade and rollback once', 'Reverify downloaded artifact bytes'])],
    legacyUpgrade: [...['core', 'commands', 'legacy-entry', 'clean-upgrade-entry', 'workspace-success-entry', 'workspace-fault-entry'].flatMap((group) => repeat(`V2.5 ${group} contracts run`,
